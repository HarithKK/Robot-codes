...
#define CENTER_MAIN_SERVO_POSITION 135
...
void goUntilEndPosition(){
  motorBrake();
  delay(200);
   int position_ =50,Counter=100; 

   rEncoderCount=0;
   rEncoderCount=0;
   
  while(rEncoderCount < position_ && lEncoderCount< position_){
     if(rEncoderCount>lEncoderCount)
        lMotorForward(Counter);
     else if(lEncoderCount>rEncoderCount)
        rMotorForward(Counter);
     else
        moveForward(Counter,Counter);   
  }
  motorBrake();
  delay(200);
  motorStop();
