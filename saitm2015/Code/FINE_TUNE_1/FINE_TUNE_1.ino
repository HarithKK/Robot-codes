...
#define R_ANGLE_135 (sensorReadings[3]==1)&&(sensorReadings[4]==0)  &&(sensorReadings[5]==1))
#define L_ANGLE_135 (sensorReadings[0]==1)&&(sensorReadings[1]==0)  &&(sensorReadings[2]==1))
...
void position2Detect(){
  lEncoderCount=0;
  
  while(lEncoderCount<6){
  ...
  passLeftT();
  motorStop();
  while(!((sensorReadings[3]==1) &&(sensorReadings[5]==1))){
   doPID();
   updateSensorArray();
  }
  turn135RightSens();
...
void doFineTune(){
  ...
  updateSensorArray();
    ...
    /*while(!(sensorReadings[0]==1 || sensorReadings[1]==1) ){
      updateSensorArray();
      lMotorReverse(120);
      rMotorForward(120);
    }
    
    while(!(sensorReadings[4]==1 || sensorReadings[5]==1 )){
      updateSensorArray();
      rMotorForward(120);
      rMotorReverse(120);
    }
...
void process_P_posision(){  
 ...
 /*
  ...
  while(!((sensorReadings[5]==1)  &&(sensorReadings[3]==1))){
     doPID();
     updateSensorArray();
  }
   stopMotors();*/
  ...
  while(!((sensorReadings[0]==1)&&(sensorReadings[1]==0)  &&(sensorReadings[2]==1))){
    doPID();
    updateSensorArray();
  }
...
void turn90Right(){
  ...
  */
  lEncoderCount=0;
    ...
    while(!(sensorReadings[4]==1 || sensorReadings[3]==1 || sensorReadings[2]==1 )){
      updateSensorArray();
      rMotorReverse(120);
      lMotorForward(120);
    }
...
void turn90Left(){
   /* 
    ...
    while(!(sensorReadings[1]==1 || sensorReadings[3]==1 || sensorReadings[2]==1 )){
      updateSensorArray();
      lMotorReverse(120);
      rMotorForward(120);
    }
...
void turn135LeftSens(){
    ...
    while(!( sensorReadings[2]==1 || sensorReadings[1]==1 || sensorReadings[0]==1)){
      updateSensorArray();
      rMotorForward(120);
      lMotorReverse(120);
    }
...
void process_Q_posision(){
  ...
  while(!((sensorReadings[0]==1 && sensorReadings[1]==0 && sensorReadings[2]==1) || (sensorReadings[0]==1 && sensorReadings[1]==0 && sensorReadings[2]==1))){
    doPID();
    updateSensorArray();  
  }
